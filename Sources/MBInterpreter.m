...
- (NSString *)key:(NSArray *)indices {
    ...
    for(NSUInteger i=0;i<indices.count;i++){NSInteger n=[indices[i]integerValue];
        if(n<self.lowerBound||n>[self.dimensions[i]integerValue])return nil;
...
@property NSMutableData *randomData;
@property NSUInteger recordLength;
@property NSMutableArray<NSDictionary *> *fields;
@end
@implementation MBFile @end
...
@property (strong) id<MBPlatform> platform;
@property NSArray<NSString *> *lines;
@property NSMutableDictionary<NSString *, NSDictionary *> *procedures;
@property NSMutableArray *dataItems;
@property NSMutableDictionary<NSString *, NSNumber *> *dataLabels;
@property NSUInteger dataIndex;
@property NSInteger optionBase;
@property NSMutableDictionary<NSNumber *, MBFile *> *files;
@property NSMutableDictionary<NSString *, NSNumber *> *labelLines;
@property NSMutableArray<NSNumber *> *areaPoints;
@property NSMutableDictionary<NSNumber *, NSNumber *> *waveforms;
@property NSMutableDictionary<NSNumber *, NSNumber *> *memory;
@property NSNumber *errorHandlerLine;
        ...
        if ([[NSCharacterSet letterCharacterSet] characterIsMember:c] || c=='_') {
            NSUInteger b=i++; while(i<s.length) { c=[s characterAtIndex:i];
                if (![[NSCharacterSet alphanumericCharacterSet] characterIsMember:c] && c!='_' && c!='.' &&
                    ![[NSCharacterSet characterSetWithCharactersInString:@"$%&!#"]characterIsMember:c]) break; i++; }
            [a addObject:[s substringWithRange:NSMakeRange(b,i-b)]]; continue;
        }
...
- (NSString *)peek { return self.pos < self.tokens.count ? self.tokens[self.pos] : nil; }
...
- (id)primary {
    ...
    if ([self take:@"("]) {
        ...
        id candidate=self.vars[[t uppercaseString]];
        if([candidate isKindOfClass:[MBArray class]])return [(MBArray *)candidate valueAt:args];
        return [self.owner call:t args:args variables:self.vars error:nil] ?: @0;
    }
...
- (NSString *)usingPattern:(NSString *)pattern values:(NSArray *)values {
    ...
    NSRegularExpression *numbers=[NSRegularExpression regularExpressionWithPattern:@"[+#*]*#[#*,]*(?:\\.#+)?[+-]*" options:0 error:nil];
    NSUInteger cursor=0;for(NSTextCheckingResult *m in [numbers matchesInString:pattern options:0 range:NSMakeRange(0,pattern.length)]){
        ...
        NSString *field=[pattern substringWithRange:m.range];id value=valueIndex<values.count?values[valueIndex++]:@0;
    ...
    while(valueIndex<values.count)[out appendString:MBString(values[valueIndex++])];return out;
}
- (NSUInteger)findEndFrom:(NSUInteger)pc open:(NSString *)open close:(NSArray *)closes elseAt:(NSUInteger *)elseAt {
    ...
    for(NSUInteger i=pc+1;i<self.lines.count;i++){NSString *u=[MBTrim(self.lines[i]) uppercaseString];
...
- (BOOL)executeFrom:(NSUInteger)start to:(NSUInteger)end variables:(NSMutableDictionary *)vars
    ...
    for(NSUInteger pc=start;pc<end&&!self.stopped;pc++){
        self.currentLine=pc;
        for(NSMutableDictionary *object in self.objects.allValues)if([object[@"MOVING"]boolValue]){
            object[@"VX"]=@([object[@"VX"]doubleValue]+[object[@"AX"]doubleValue]);
            object[@"VY"]=@([object[@"VY"]doubleValue]+[object[@"AY"]doubleValue]);
            object[@"X"]=@([object[@"X"]doubleValue]+[object[@"VX"]doubleValue]);
            object[@"Y"]=@([object[@"Y"]doubleValue]+[object[@"VY"]doubleValue]);
        }
        ...
        for(NSUInteger ai=0;ai<objectKeys.count&&!collisionNow;ai++)for(NSUInteger bi=ai+1;bi<objectKeys.count;bi++){
            NSDictionary *a=self.objects[objectKeys[ai]],*b=self.objects[objectKeys[bi]];
            NSRect ar=NSMakeRect([a[@"X"]doubleValue],[a[@"Y"]doubleValue],MAX(1,[a[@"W"]doubleValue]),MAX(1,[a[@"H"]doubleValue]));
            NSRect br=NSMakeRect([b[@"X"]doubleValue],[b[@"Y"]doubleValue],MAX(1,[b[@"W"]doubleValue]),MAX(1,[b[@"H"]doubleValue]));
        ...
        NSString *raw=MBTrim(self.lines[pc]); NSString *u=[raw uppercaseString];
        ...
        if([u isEqual:@"RESTORE"]||[u hasPrefix:@"RESTORE "]){
            ...
            if(label.length&&!self.dataLabels[label]){if(error)*error=[self err:[NSString stringWithFormat:@"Unknown DATA label '%@'",label] line:pc];return NO;}
            self.dataIndex=label.length?[self.dataLabels[label]unsignedIntegerValue]:0;continue;
        }
        ...
        if([u hasPrefix:@"ON ERROR GOTO "]){
            ...
            self.errorHandlerLine=[target isEqual:@"0"]?nil:self.labelLines[target];
        ...
        if([u isEqual:@"RESUME"]||[u hasPrefix:@"RESUME "]){
            ...
            else if(target.length)self.resumeTarget=self.labelLines[target];
            else self.resumeTarget=@(self.faultLine);
            return YES;
        }
        if([u hasPrefix:@"OPEN "]){
            NSRegularExpression *random=[NSRegularExpression regularExpressionWithPattern:@"(?i)^OPEN\\s+(.+?)\\s+AS\\s+#?([0-9]+)\\s+LEN\\s*=\\s*([0-9]+)$" options:0 error:nil];
            ...
            if(randomMatch){NSString *path=MBString([self evaluate:[raw substringWithRange:[randomMatch rangeAtIndex:1]]variables:vars]);
                ...
                NSData *existing=[NSData dataWithContentsOfFile:path];file.randomData=existing?[existing mutableCopy]:[NSMutableData data];self.files[number]=file;continue;}
            NSRegularExpression *r=[NSRegularExpression regularExpressionWithPattern:@"(?i)^OPEN\\s+(.+?)\\s+FOR\\s+(INPUT|OUTPUT|APPEND)\\s+AS\\s+#?([0-9]+)$" options:0 error:nil];
            ...
            }self.files[number]=file;continue;
        }
        if([u isEqual:@"CLOSE"]||[u hasPrefix:@"CLOSE "]){
            ...
            for(id target in targets){NSNumber *number=[target isKindOfClass:[NSNumber class]]?target:@([[[target description]stringByReplacingOccurrencesOfString:@"#" withString:@""]integerValue]);
                MBFile *file=self.files[number];if(file&&![file.mode isEqual:@"INPUT"]){NSError *writeError=nil;BOOL wrote;
        ...
        if([u hasPrefix:@"FIELD #"]){
            ...
            MBFile *file=self.files[number];if(!file||![file.mode isEqual:@"RANDOM"]){if(error)*error=[self err:@"FIELD requires a random file" line:pc];return NO;}
            [file.fields removeAllObjects];for(NSString *field in [self parts:[raw substringFromIndex:comma.location+1]]){
                NSRegularExpression *fr=[NSRegularExpression regularExpressionWithPattern:@"(?i)^\\s*([0-9]+)\\s+AS\\s+([A-Z][A-Z0-9_$]*)\\s*$" options:0 error:nil];
        ...
        if([u hasPrefix:@"GET #"]||[u hasPrefix:@"PUT #"]){
            ...
            MBFile *file=self.files[number];NSUInteger record=p.count>1?MAX(1,[[self evaluate:p[1]variables:vars]integerValue]):file.index+1;NSUInteger offset=(record-1)*file.recordLength;
        ...
        if([u hasPrefix:@"PRINT #"]||[u hasPrefix:@"WRITE #"]){
            ...
            MBFile *file=self.files[number];if(!file||[file.mode isEqual:@"INPUT"]){if(error)*error=[self err:@"File is not open for output" line:pc];return NO;}
        ...
        if([u hasPrefix:@"INPUT #"]||[u hasPrefix:@"LINE INPUT #"]){
            ...
            MBFile *file=self.files[number];if(!file||file.index>=file.lines.count){if(error)*error=[self err:@"End of file" line:pc];return NO;}
        ...
        if([u hasPrefix:@"IF "]&&[u hasSuffix:@" THEN"]){
            ...
            if(!MBTruth([self evaluate:cond variables:vars])){
                if(alt!=NSNotFound&&[[MBTrim(self.lines[alt])uppercaseString]hasPrefix:@"ELSEIF "]){loops[[NSString stringWithFormat:@"I%lu",(unsigned long)alt]]=@YES;pc=alt-1;}
        ...
        if([u hasPrefix:@"IF "]&&[u containsString:@" THEN "]){
            ...
            if(chosen.length){NSString *target=[MBTrim(chosen)uppercaseString];
                ...
                NSNumber *line=self.labelLines[target];if(line){pc=line.unsignedIntegerValue-1;continue;}
                NSArray *saved=self.lines;NSMutableArray *temporary=[self.lines mutableCopy];temporary[pc]=chosen;self.lines=temporary;
        ...
        if([u hasPrefix:@"ELSEIF "]&&[u hasSuffix:@" THEN"]){
            ...
            if(!loops[marker]){pc=[self findEndFrom:pc open:@"~" close:@[@"END IF",@"ENDIF"] elseAt:NULL];continue;}
            ...
            if(!MBTruth([self evaluate:cond variables:vars])){NSUInteger next;
                ...
                if(next!=NSNotFound&&[[MBTrim(self.lines[next])uppercaseString]hasPrefix:@"ELSEIF "]){loops[[NSString stringWithFormat:@"I%lu",(unsigned long)next]]=@YES;pc=next-1;}
        ...
        if([u hasPrefix:@"WHILE "]){
            ...
            else loops[[NSString stringWithFormat:@"W%lu",(unsigned long)[self findEndFrom:pc open:@"WHILE " close:@[@"WEND"] elseAt:NULL]]]=@(pc);
            continue;
        }
        if([u isEqual:@"WEND"]){ NSNumber *back=loops[[NSString stringWithFormat:@"W%lu",(unsigned long)pc]]; if(back)pc=back.unsignedIntegerValue-1; continue; }
        if([u hasPrefix:@"FOR "]){
            NSRegularExpression *re=[NSRegularExpression regularExpressionWithPattern:@"(?i)^FOR\\s+([A-Z_$][A-Z0-9_$]*)\\s*=\\s*(.*?)\\s+TO\\s+(.*?)(?:\\s+STEP\\s+(.*))?$" options:0 error:nil];
        ...
        if([u hasPrefix:@"ON TIMER("]&&[u containsString:@") GOSUB "]){
            ...
            timerTarget=self.labelLines[[[raw substringFromIndex:NSMaxRange(close)]uppercaseString]];timerNext=[NSDate timeIntervalSinceReferenceDate]+timerInterval;continue;
        }
        ...
        if([u hasPrefix:@"ON MOUSE GOSUB "]){mouseTarget=self.labelLines[[[raw substringFromIndex:15]uppercaseString]];continue;}
        if([u hasPrefix:@"ON KEY GOSUB "]){keyTarget=self.labelLines[[[raw substringFromIndex:13]uppercaseString]];continue;}
        if([u hasPrefix:@"ON MENU GOSUB "]){menuTarget=self.labelLines[[[raw substringFromIndex:14]uppercaseString]];continue;}
        if([u hasPrefix:@"ON COLLISION GOSUB "]){collisionTarget=self.labelLines[[[raw substringFromIndex:19]uppercaseString]];continue;}
        ...
        if([u hasPrefix:@"MENU "]&&![u isEqual:@"MENU RESET"]){
            NSArray *p=[self parts:[raw substringFromIndex:5]];if(p.count>=3)[self.platform setMenu:[[self evaluate:p[0]variables:vars]integerValue]
                item:[[self evaluate:p[1]variables:vars]integerValue] state:[[self evaluate:p[2]variables:vars]integerValue]
                title:p.count>3?MBString([self evaluate:p[3]variables:vars]):@""];continue;
        }
        if([u hasPrefix:@"ON "]&&([u containsString:@" GOTO "]||[u containsString:@" GOSUB "])){
            ...
            if(choice>=1&&choice<=(NSInteger)targets.count){NSNumber *line=self.labelLines[[MBTrim(targets[choice-1])uppercaseString]];
        ...
        if([u hasPrefix:@"GOTO "]||[u hasPrefix:@"GOSUB "]){
            ...
            NSNumber *line=self.labelLines[target];if(!line){if(error)*error=[self err:[NSString stringWithFormat:@"Unknown label '%@'",target] line:pc];return NO;}
        ...
        if([u hasPrefix:@"AREA "]){
            ...
            NSArray *p=[self parts:tail];if(p.count>=2){[self.areaPoints addObject:[self evaluate:p[0] variables:vars]];[self.areaPoints addObject:[self evaluate:p[1] variables:vars]];}continue;
        }
        ...
        if([u hasPrefix:@"WAVE "]){
            NSArray *p=[self parts:[raw substringFromIndex:5]];if(p.count>=2)self.waveforms[@([[self evaluate:p[0]variables:vars]integerValue])]=@([[self evaluate:p[1]variables:vars]integerValue]);continue;
        }
        if([u hasPrefix:@"SOUND "]){
            ...
            NSInteger voice=p.count>3?[[self evaluate:p[3]variables:vars]integerValue]:0;double volume=p.count>2?[[self evaluate:p[2]variables:vars]doubleValue]:1;
            if(volume>1)volume/=255.0;[self.platform playTone:[[self evaluate:p[0]variables:vars]doubleValue] duration:[[self evaluate:p[1]variables:vars]doubleValue]
                volume:volume voice:voice waveform:[self.waveforms[@(voice)]integerValue]];continue;
        }
        ...
        if([u hasPrefix:@"PROCESS RUN "]){
            ...
            for(NSUInteger i=1;i<p.count;i++)[args addObject:MBString([self evaluate:p[i] variables:vars])];
            [self.platform runProcess:MBString([self evaluate:p[0] variables:vars]) arguments:args];continue;
        }
        ...
        if([u hasPrefix:@"POKE "]||[u hasPrefix:@"POKEW "]||[u hasPrefix:@"POKEL "]){
            ...
            if(p.count>=2)self.memory[@([[self evaluate:p[0]variables:vars]integerValue])]=@([[self evaluate:p[1]variables:vars]integerValue]);continue;
        }
        if([u hasPrefix:@"SWAP "]){
            NSArray *p=[self parts:[raw substringFromIndex:5]];if(p.count==2){NSString *a=[p[0]uppercaseString],*b=[p[1]uppercaseString];id temp=vars[a]?:@0;vars[a]=vars[b]?:@0;vars[b]=temp;}continue;
        }
        if([u hasPrefix:@"WAIT "]){
            NSArray *p=[self parts:[raw substringFromIndex:5]];NSInteger address=[[self evaluate:p[0]variables:vars]integerValue],mask=p.count>1?[[self evaluate:p[1]variables:vars]integerValue]:-1;
            while(!self.stopped&&([self.memory[@(address)]integerValue]&mask)==0)[NSThread sleepForTimeInterval:.001];continue;
        }
        ...
        if([u hasPrefix:@"OBJECT."]){
            NSRegularExpression *set=[NSRegularExpression regularExpressionWithPattern:@"(?i)^OBJECT\\.([A-Z]+)\\s*\\((.*?)\\)\\s*=\\s*(.*)$" options:0 error:nil];
            ...
            if(m){NSString *property=[[raw substringWithRange:[m rangeAtIndex:1]]uppercaseString];NSNumber *number=@([[self evaluate:[raw substringWithRange:[m rangeAtIndex:2]]variables:vars]integerValue]);
                NSMutableDictionary *object=self.objects[number]?:[NSMutableDictionary dictionary];object[property]=[self evaluate:[raw substringWithRange:[m rangeAtIndex:3]]variables:vars];self.objects[number]=object;continue;}
            NSRegularExpression *action=[NSRegularExpression regularExpressionWithPattern:@"(?i)^OBJECT\\.(ON|OFF|START|STOP|CLOSE)\\s*\\(?\\s*([0-9]+)\\s*\\)?$" options:0 error:nil];
            ...
            if(m){NSString *command=[[raw substringWithRange:[m rangeAtIndex:1]]uppercaseString];NSNumber *number=@([[raw substringWithRange:[m rangeAtIndex:2]]integerValue]);
                if([command isEqual:@"CLOSE"])[self.objects removeObjectForKey:number];else{NSMutableDictionary *object=self.objects[number]?:[NSMutableDictionary dictionary];
                    object[@"ON"]=@([command isEqual:@"ON"]||[command isEqual:@"START"]);object[@"MOVING"]=@([command isEqual:@"START"]);self.objects[number]=object;}continue;}
        }
        NSRange eq=[raw rangeOfString:@"="]; if(eq.location!=NSNotFound){
            ...
            if(par.location!=NSNotFound&&[left hasSuffix:@")"]){
                NSString *name=[[MBTrim([left substringToIndex:par.location])uppercaseString]copy];MBArray *array=vars[name];
                ...
                if(leftSet||rightSet){NSUInteger width=MBString(vars[name]).length;for(MBFile *file in self.files.allValues)for(NSDictionary *field in file.fields)
                        if([field[@"name"]isEqual:name])width=[field[@"length"]unsignedIntegerValue];
        ...
        else {NSArray *bits=[raw componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];name=bits[0];NSString *tail=raw.length>name.length?MBTrim([raw substringFromIndex:name.length]):@"";argTexts=[self parts:tail];}
...
- (id)call:(NSString *)name args:(NSArray *)args variables:(NSMutableDictionary *)caller error:(NSError **)error {
    ...
    if([key isEqual:@"UBOUND"]&&[args.firstObject isKindOfClass:[MBArray class]]){
        MBArray *a=args.firstObject;NSInteger d=args.count>1?[args[1]integerValue]-1:0;
        return d>=0&&d<(NSInteger)a.dimensions.count?a.dimensions[d]:@0;
    }
    ...
    if([key isEqual:@"LEFT$"]){NSUInteger count=MIN(s.length,(NSUInteger)MAX(0,[args[1]integerValue]));return [s substringToIndex:count];}
    if([key isEqual:@"RIGHT$"]){NSUInteger count=MIN(s.length,(NSUInteger)MAX(0,[args[1]integerValue]));return [s substringFromIndex:s.length-count];}
    if([key isEqual:@"MID$"]){NSInteger start=MAX(1,[args[1]integerValue]);NSUInteger location=MIN(s.length,(NSUInteger)(start-1));
        NSUInteger count=args.count>2?MIN(s.length-location,(NSUInteger)MAX(0,[args[2]integerValue])):s.length-location;
    ...
    if([key isEqual:@"STRING$"]){NSUInteger count=MAX(0,[args.firstObject integerValue]);NSString *ch=args.count>1?MBString(args[1]):@" ";
    ...
    if([key isEqual:@"INSTR"]){NSUInteger start=args.count>2?MAX(1,[args[0]integerValue]):1;NSString *hay=MBString(args[args.count>2?1:0]);NSString *needle=MBString(args[args.count>2?2:1]);
    ...
    if([key isEqual:@"INPUT$"]){NSUInteger count=MAX(0,[args.firstObject integerValue]);NSString *value;
        if(args.count>1){MBFile *file=self.files[@([args[1]integerValue])];value=file.index<file.lines.count?file.lines[file.index++]:@"";}
    ...
    if([@[@"EOF",@"LOC",@"LOF"] containsObject:key]){
        MBFile *file=self.files[@([args.firstObject integerValue])];if(!file)return @(-1);
        ...
        return attributes[NSFileSize]?:@(file.output.length);
    }
    if([@[@"PEEK",@"PEEKW",@"PEEKL"]containsObject:key])return self.memory[@([args.firstObject integerValue])]?:@0;
    if([key isEqual:@"FRE"])return @(NSIntegerMax);
    if([@[@"POS",@"LPOS",@"CSRLIN",@"POINT"]containsObject:key])return @0;
    if([key hasPrefix:@"OBJECT."]){NSMutableDictionary *object=self.objects[@([args.firstObject integerValue])];
        return object[[key substringFromIndex:7]]?:@0;}
    if([key isEqual:@"COLLISION"]||[key isEqual:@"OBJECT.HIT"]){
        NSDictionary *a=self.objects[@([args.firstObject integerValue])],*b=args.count>1?self.objects[@([args[1]integerValue])]:nil;
        if(!a||!b)return @0;NSRect ar=NSMakeRect([a[@"X"]doubleValue],[a[@"Y"]doubleValue],MAX(1,[a[@"W"]doubleValue]),MAX(1,[a[@"H"]doubleValue]));
        NSRect br=NSMakeRect([b[@"X"]doubleValue],[b[@"Y"]doubleValue],MAX(1,[b[@"W"]doubleValue]),MAX(1,[b[@"H"]doubleValue]));
...
- (BOOL)runSource:(NSString *)source error:(NSError **)error {
    ...
    for(NSString *original in [source componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]]){
        ...
        NSRegularExpression *numbered=[NSRegularExpression regularExpressionWithPattern:@"^([0-9]+)\\s+(.*)$" options:0 error:nil];
        NSTextCheckingResult *match=[numbered firstMatchInString:trim options:0 range:NSMakeRange(0,trim.length)];
        if(match){self.labelLines[[trim substringWithRange:[match rangeAtIndex:1]]]=@(prepared.count);
            line=[trim substringWithRange:[match rangeAtIndex:2]];trim=MBTrim(line);}
        if([trim hasSuffix:@":"]){NSString *label=[[MBTrim([trim substringToIndex:trim.length-1])uppercaseString]copy];self.labelLines[label]=@(prepared.count);}
    ...
    for(NSUInteger i=0;i<self.lines.count;i++){NSString *s=MBTrim(self.lines[i]);NSTextCheckingResult *m=[re firstMatchInString:s options:0 range:NSMakeRange(0,s.length)];if(!m)continue;
        ...
        self.procedures[name]=@{@"params":[self parts:params],@"start":@(i+1),@"end":@(end)};i=end;
    }
    ...
    for(NSString *line in self.lines){
        ...
        if([u hasPrefix:@"DATA"]&&(u.length==4||[u characterAtIndex:4]==' ')){
            if(pendingLabel){self.dataLabels[pendingLabel]=@(self.dataItems.count);pendingLabel=nil;}
    ...
    if(!ok&&self.errorHandlerLine){
        self.faultLine=self.currentLine;NSError *fault=error?*error:nil;globals[@"ERR"]=@(fault.code);globals[@"ERL"]=@(self.faultLine+1);
